...
Linux 下将 `"command": "py"` 和 `"args": ["-3.12", ...]` 替换为 `"command": "python3"` 和 `"args": ["-m", "youvedio", "mcp"]`。
...
注意：使用 `py -3.12 -m ruff` / `py -3.12 -m mypy` / `py -3.12 -m pytest` 如果默认 Python 不是 3.12。Linux 下将 `py -3.12` 替换为 `python3`。
